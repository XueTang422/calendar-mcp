...
async function createNewSession(
    ...
    const transport = new StreamableHTTPServerTransport({
        ...
        onsessioninitialized: (sessionId) => {
            ...
            console.log('New Google Calendar session created:', sessionId);
        }
    });
    ...
    transport.onclose = () => {
        if (transport.sessionId) {
            ...
            console.log('Google Calendar session closed:', transport.sessionId);
        }
    };
...
function logServerStart(config: Config): void {
    ...
    console.log(`Google Calendar MCP Server listening on ${displayUrl}`);
